...
# Ensure repo root and IAD code on path.
_THIS_DIR = os.path.dirname(os.path.abspath(__file__))
_REPO_ROOT = os.path.abspath(os.path.join(_THIS_DIR, "..", ".."))
for p in (_REPO_ROOT, os.path.join(_THIS_DIR, "IAD"), os.path.join(_REPO_ROOT, "train_models")):
...
# Try loading IAD modules (prefer bundled attack/eval_bd/IAD, fallback to train_models/IAD).
try:
    from config import get_arguments  # type: ignore
    from dataloader import get_dataloader  # type: ignore
    from utils import progress_bar  # type: ignore
    ...
    from train_models.IAD.utils import progress_bar  # type: ignore
...
def main():
    ...
    # Allow checkpoint passed directly via --checkpoints
    ...
    path_model = _resolve_ckpt_path(opt)
