...
def main(args):
    # Load the config
    config_filepath = os.path.join(os.path.dirname(__file__), 'configs', f'{args.attack}.json')
    with open(config_filepath, 'r') as f:
        config = json.load(f)

    # Create a directory for the model
    model_dir = os.path.join(os.path.dirname(__file__), f'model_dir_{args.attack}')
