...
# Ensure DFST package paths (prefer local models/backdoors over repo root)
_THIS_DIR = os.path.dirname(os.path.abspath(__file__))
_DFST_ROOT = _THIS_DIR
for p in (_DFST_ROOT,):
    if os.path.isdir(p) and p not in sys.path:
        sys.path.insert(0, p)

from models import *  # noqa: E402,F401,F403
from backdoors import *  # noqa: E402,F401,F403
...
# Get dataset
def get_dataset(dataset, datadir='data', train=True, augment=True, download=False):
    transform = get_transform(dataset, augment=train & augment)
    
    if dataset == 'cifar10':
        dataset = datasets.CIFAR10(datadir, train, download=download, transform=transform)
