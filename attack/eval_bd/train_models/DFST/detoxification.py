...
_DFST_MODELS = os.path.join(_THIS_DIR, "models")
for p in (_THIS_DIR, _DFST_MODELS, _REPO_ROOT):
    if os.path.isdir(p) and p not in sys.path:
        sys.path.insert(0, p)

# Robust import for FeatureInjector (script or package modes)
FeatureInjector = None
for mod in (
    "models.genr",
    "train_models.DFST.models.genr",
    "attack.eval_bd.train_models.DFST.models.genr",
):
    try:
        _m = __import__(mod, fromlist=["FeatureInjector"])
        FeatureInjector = getattr(_m, "FeatureInjector", None)
        if FeatureInjector is not None:
            break
    except Exception:
        continue
if FeatureInjector is None:
    import importlib.util
    genr_path = os.path.join(_DFST_MODELS, "genr.py")
    if os.path.isfile(genr_path):
        spec = importlib.util.spec_from_file_location("dfst_genr", genr_path)
        if spec and spec.loader:
            mod = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(mod)
            FeatureInjector = getattr(mod, "FeatureInjector", None)
if FeatureInjector is None:
    raise ImportError("Cannot import FeatureInjector from DFST models.genr; check sys.path and package setup.")
